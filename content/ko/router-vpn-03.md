...
[[ko/router-vpn-02|2편]]에서 VPN 서버 구축을 마쳤다. 이제 VPN 터널이 뚫려 있으니, 그 터널 끝에 무언가를 놓을 차례다. 맥북이다.

## NAS는 어떻게 됐나

처음엔 NAS도 고려했다. LG U+ 공유기 뒷면에 USB 포트가 있어서 외장 하드를 연결해보려 했는데, 통신사 공유기는 소프트웨어적으로 해당 기능을 막아둔 경우가 많다고 한다. 실제로 관리 페이지에서 확인해보니 USB 서비스 메뉴 자체가 없었다.

Archer C6는 USB 포트가 없다. 별도 NAS 기기를 사는 건 당장 필요하지 않다고 판단했다.

결론은 **맥북을 직접 서버로 쓰는 것**. 어차피 집에 있고, 성능은 공유기 USB NAS와 비교가 안 된다.

목표는 두 가지다:
- **SMB 파일 공유**: 아이폰에서 맥북 파일에 접근 (NAS처럼)
- **VNC 원격 제어**: 아이폰에서 맥북 화면을 보고 제어
...
현재 집에 있는 IoT 기기는 Matter 스마트 스위치 하나다. 아직 게스트 네트워크에 연결하지 않았지만, 조만간 삼성 스마트허브 등 기기가 늘어나면 그때부터 게스트 네트워크를 IoT 전용으로 운영할 계획이다. 메인 네트워크에 있는 맥북과 완전히 격리된 상태로.

이 시리즈의 기술적인 내용은 여기서 마무리다. 다음 편에서는 이 작업 전체를 통해 Gemini를 어떻게 학습 도구로 활용했는지를 따로 다룬다.

---

| 편 | 제목 |
|---|---|
| ← 이전 | [[ko/router-vpn-02\|2편: VPN 서버 구축]] |
| 시리즈 처음 | [[ko/router-vpn-01\|1편: 왜 만들었고, 어떻게 설계했나]] |
