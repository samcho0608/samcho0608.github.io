...
- VPN 연결 이후 실제로 쓸 서비스(SMB, VNC)까지 확장하고 싶은 사람
- macOS에서 DHCP 예약이 안 먹는 문제를 겪는 사용자

이 글에서는 맥북을 홈서버로 붙이면서 생긴 Private Wi-Fi Address 이슈를 해결하고, 파일 공유/원격 제어를 외부망에서 검증한다.

VPN 터널이 뚫렸으니 그 끝에 뭔가를 붙여야 한다. Gemini가 먼저 제안한 건 LG 공유기 USB 포트에 외장 하드를 붙이는 방법이었다. 시도했는데 통신사 펌웨어에서 막혀 있었다. Gemini에 물어보니 통신사 공유기는 보안 정책상 USB 저장 기능을 제한하는 경우가 많다고 했다. Archer C6는 USB 포트 자체가 없고, 별도 NAS는 비용이 크다 — Gemini가 다음으로 짚어준 건 **맥북**이었다.

처음엔 SSH 접속만 생각했는데, Gemini가 파일 공유는 SMB를, 화면 제어는 VNC를 제안했다. 아이폰에서 쓰기엔 GUI가 있는 편이 훨씬 낫다고.
...
옆의 `i` 아이콘을 눌러:
- 공유할 폴더를 추가하고, 폴더별로 접근 가능한 **사용자**와 **권한**(읽기 전용 / 읽기&쓰기)을 지정한다.
- **옵션**에서 'SMB를 사용하여 파일 및 폴더 공유'에 체크하고, 계정 목록에서 접근 허용할 macOS 계정도 체크한다. 이 단계에서 맥북 로그인 암호를 재입력해야 한다 — SMB 인증용 암호 해시를 별도로 저장하는 과정이다.
...
**원격 관리 `i` 아이콘 → 옵션**에서 모든 항목을 체크하고, **컴퓨터 설정**에서 VNC 전용 암호를 설정한다. 이 암호는 macOS 로그인 암호와 별개로, VNC 접속 전용으로 쓰인다. 접근 허용 대상은 '모든 사용자' 또는 특정 계정으로 제한할 수 있다.
...
graph TD
    Internet["🌐 인터넷"]
    LG["LG U+ 공유기<br/>포트 포워딩 UDP 1194 → 192.168.219.xxx<br/>DHCP 고정: C6 = 192.168.219.xxx"]
